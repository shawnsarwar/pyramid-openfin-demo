...
function removeTemplateByName(templateStoreKey, name) {
    const templates = getTemplates(templateStoreKey);
    const updatedTemplates = templates.filter((t) => t.name !== name);

    localStorage.setItem(templateStoreKey, JSON.stringify(updatedTemplates));
...
export { storeTemplate, getTemplates, getTemplateByName, onStoreUpdate, removeTemplateByName };
